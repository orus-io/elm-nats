...
var $author$project$Nats$Protocol$parseString = F2(
	function (partialOp, str) {
		if (partialOp.$ === 'PartialMessage') {
			var partial = partialOp.a;
			var data = A2($elm$core$String$append, partial.data, str);
			var newDataSize = $elm$core$String$length(data);
			if (_Utils_eq(newDataSize, partial.size + 2) && A2($elm$core$String$endsWith, $author$project$Nats$Protocol$cr, data)) {
				return $elm$core$Result$Ok(
					_Utils_Tuple2(
						_List_fromArray(
							[
				...
				if (_Utils_cmp(newDataSize, partial.size + 2) > 0) {
					var _v1 = A2(
						$author$project$Nats$Protocol$parseString,
						$author$project$Nats$Protocol$initialParseState,
						A2($elm$core$String$dropLeft, partial.size + 2, data));
					if (_v1.$ === 'Err') {
						var err = _v1.a;
						return $elm$core$Result$Err(err);
					} else {
							...
							_Utils_Tuple2(
								A2(
									...
									A2(
										...
										partial.sid,
										{
											data: A2($elm$core$String$left, partial.size, data),
											replyTo: partial.replyTo,
											size: partial.size,
											subject: partial.subject
										}),
			...
			var partial = partialOp.a;
			var _v3 = $author$project$Nats$Protocol$splitFirstLine(str);
			if (_v3.b.$ === 'Nothing') {
				var firstLine = _v3.a;
				var _v4 = _v3.b;
				return $elm$core$Result$Ok(
				...
				if (_v5.$ === 'Err') {
					...
					if (_v5.a.$ === 'MSG') {
						...
						if (_Utils_eq(
							...
							msg.size + 2)) {
							return A2($elm$core$String$endsWith, $author$project$Nats$Protocol$cr, tail) ? $elm$core$Result$Ok(
								_Utils_Tuple2(
									_List_fromArray(
										[
											A2(
											...
											sid,
											{
												data: A2($elm$core$String$dropRight, 2, tail),
												replyTo: msg.replyTo,
												size: msg.size,
												subject: msg.subject
											})
										]),
									$author$project$Nats$Protocol$initialParseState)) : $elm$core$Result$Err('message payload size mismatch');
						} else {
							if (_Utils_cmp(
								$elm$core$String$length(tail),
								msg.size + 2) > 0) {
								if (A2(
									...
									A2($elm$core$String$dropLeft, msg.size, tail))) {
									var _v7 = A2(
										$author$project$Nats$Protocol$parseString,
										$author$project$Nats$Protocol$initialParseState,
										A2($elm$core$String$dropLeft, msg.size + 2, tail));
									if (_v7.$ === 'Err') {
										var err = _v7.a;
										return $elm$core$Result$Err(err);
									} else {
											...
											_Utils_Tuple2(
												A2(
													...
													A2(
														...
														sid,
														{
															data: A2($elm$core$String$left, msg.size, tail),
															replyTo: msg.replyTo,
															size: msg.size,
															subject: msg.subject
														}),
								...
								return $elm$core$Result$Ok(
									_Utils_Tuple2(
										_List_Nil,
										$author$project$Nats$Protocol$PartialMessage(
											{data: tail, replyTo: msg.replyTo, sid: sid, size: msg.size, subject: msg.subject})));
							}
						}
					} else {
						var op = _v5.a;
						var _v9 = A2($author$project$Nats$Protocol$parseString, $author$project$Nats$Protocol$initialParseState, tail);
						if (_v9.$ === 'Err') {
							var err = _v9.a;
							return $elm$core$Result$Err(err);
						} else {
...
var $author$project$Nats$Config$string = F2(
	function (parentMsg, ports) {
		return $author$project$Nats$Internal$Types$Config(
			{debug: false, fromPortMessage: $elm$core$Result$Ok, mode: 'text', onError: $elm$core$Maybe$Nothing, parentMsg: parentMsg, parse: $author$project$Nats$Protocol$parseString, ports: ports, size: $elm$core$String$length, toPortMessage: $elm$core$Basics$identity, write: $author$project$Nats$Protocol$toString});
	});
...
var $author$project$Main$natsConfig = A2(
	...
	A2(
		...
		{receive: $author$project$Main$natsReceive, send: $author$project$Main$natsSend}));
...
var $author$project$Nats$Internal$SocketState$init = F4(
	function (options, onEvent, _v0, time) {
		var socket = _v0.a;
		return {activeSubscriptions: _List_Nil, activeTrackers: _List_Nil, connectOptions: options, lastSubID: 0, nextSubscriptions: $elm$core$Dict$empty, nextTrackers: _List_Nil, onEvent: onEvent, parseState: $author$project$Nats$Protocol$initialParseState, serverInfo: $elm$core$Maybe$Nothing, socket: socket, status: $author$project$Nats$Socket$Undefined, time: time};
	});
...
var $author$project$Nats$Internal$SocketState$track = F2(
	function (marker, state) {
		return _Utils_update(
			state,
			{
				nextTrackers: A2($elm$core$List$cons, marker, state.nextTrackers)
			});
	});
				...
				if (_v6 === '') {
					...
					var _v7 = A4(
						...
						function (socket) {
							return _Utils_Tuple3(
								$elm$core$Maybe$Just(
									A2($author$project$Nats$Internal$SocketState$track, marker, socket)),
								_List_Nil,
								$elm$core$Platform$Cmd$none);
						},
						oState);
				...
				if (_v3 === '') {
					...
					var _v4 = function () {
						if (replyTo.$ === 'Just') {
							...
							return $author$project$Nats$nextInbox(oState);
						}
					}();
					var inbox = _v4.a;
					var state1 = _v4.b;
...
var $author$project$Nats$Internal$Sub$Track = function (a) {
	return {$: 'Track', a: a};
};
			...
			A2(
				...
				function (s) {
					switch (s.$) {
						...
						default:
							var marker = s.a;
							return $author$project$Nats$Internal$Sub$Track(marker);
					}
				},
				sub));
	});
...
var $author$project$Nats$Effect$map = F2(
	function (fn, effect) {
		switch (effect.$) {
				...
				return $author$project$Nats$Internal$Types$Request(
					{
						...
						onTimeout: A2($elm$core$Basics$composeR, onTimeout, fn),
						replyTo: replyTo,
						sid: sid,
						subject: subject,
						timeout: timeout
			...
			case 'CancelRequest':
				var req = effect.a;
				return $author$project$Nats$Internal$Types$CancelRequest(req);
			case 'BatchEffect':
				var list = effect.a;
		...
		return $author$project$Nats$Internal$Types$Request(
			{
				...
				replyTo: $elm$core$Maybe$Nothing,
